...
std::ostream& operator<<(std::ostream &os, const Move &move) {
	return os << ((move.side == Side::WHITE) ? "White" : "Black") << ": From "
			<< CellUtils::toChessCoordinates(move.source) << ", To "
			<< CellUtils::toChessCoordinates(move.destination) << " and the score is "
			<< move.score;
}
